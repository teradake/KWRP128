...
{
    public class NaiveLaneArrangementService : ILaneArrangementService
    {
        ...
        public NaiveLaneArrangementService(
        ...
        public async Task<double> ArrangeLanesAsync(PolygonWithHoles target, bool onlyCalculateDirection = false)
        {
            var simulators = _directions
                ...
                .Select(p => new NaiveLaneArranger(p))
                .ToArray();
            ...
            var scores = simulators
                .Select(sim =>
                {
                    sim.CreateInscribeLanes(_parameterStore.LaneWidth, _parameterStore.LapWidth);
